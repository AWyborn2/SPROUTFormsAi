...
export interface JoinLink {
  id: string;
  token: string;
  /** Path only; the screen prefixes the current origin. */
  path: string;
  label: string;
  role: string;
...
export interface JoinLinkTarget {
  orgName: string;
  label: string;
  role: string;
  usable: boolean;
...
export const joinLinksApi = {
  list: () => apiClient.get<JoinLink[]>('/team/join-links'),

  create: (input: { label?: string; expiresAt?: string | null; maxUses?: number | null }) =>
    apiClient.post<JoinLink>('/team/join-links', input),

  revoke: (id: string) => apiClient.delete<void>(`/team/join-links/${id}`),

  /** Public — no session needed; the scanner has no account yet. */
  describe: (token: string) => apiClient.get<JoinLinkTarget>(`/join/${token}`),

  accept: (token: string) =>
    apiClient.post<{ orgId: string; role: string; alreadyMember: boolean }>(`/join/${token}/accept`, {}),
};
